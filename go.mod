module github.com/cloudfoundry/php-compat-cnb

go 1.12

require (
	github.com/buildpack/libbuildpack v1.25.11
	github.com/cloudfoundry/dagger v0.0.0-20200115142400-b69a9b4eabf4
	github.com/cloudfoundry/httpd-cnb v0.0.67
	github.com/cloudfoundry/libcfbuildpack v1.91.23
	github.com/cloudfoundry/nginx-cnb v0.0.73
	github.com/cloudfoundry/php-composer-cnb v0.0.47
	github.com/cloudfoundry/php-dist-cnb v0.0.77 // indirect
	github.com/cloudfoundry/php-web-cnb v0.0.82 // indirect
	github.com/google/go-cmp v0.4.0
	github.com/onsi/gomega v1.8.1
	github.com/sclevine/spec v1.4.0
	gopkg.in/yaml.v2 v2.2.8
)
